...
def sameStreet(street1, street2):
    list1 = street1.split('&')
    list2 = street2.split('&')
    for item in list1:
        if item in list2:
...
def binarySearch(df, dataFrame, api_key):
    ...
    if len(df) <= 2 and start_address != end_address:
        return None
            ...
            sameStreet(start_address, mid_address) and sameStreet(mid_address, end_address)):
...
def df_cleanup(df):
    ...
    for i in range(1, df_len):
        if i == df_len - 1:
            break
        if df.iloc[i]['Street'] == buffer or sameStreet(df.iloc[i]['Street'], buffer) or df.iloc[i]['Street'] == "":
            df.at[i, 'Street'] = ""
        else:
